...
TT_DIV = "DIV"
TT_POW = "POW"
TT_EQ = "EQ"  # Attribuition value to
TT_LPAREN = "LPAREN"
TT_RPAREN = "RPAREN"
TT_LSQUARE = "LSQUARE"  # [
TT_RSQUARE = "RSQUARE"  # ]
...
class ListNode:
    ...
    def __init__(self, element_nodes, pos_start, pos_end):
        self.element_nodes = element_nodes  # element_nodes could be an NumberNode or StringNode, or any Node type
...
class Parser:
    ...
    def statement(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "CONTINUE"):
            ...
            return res.success(
                ContinueNode(
                    pos_start=pos_start, pos_end=self.current_tok.pos_start.copy()
                )
            )
    ...
    def power(self):
        return self.bin_op(
            atomic_func=self.call, ops=(TT_POW,), factor_func=self.factor
        )
    ...
    def atom(self):
        ...
        return res.failure(
            InvalidSyntaxError(
                ...
                pos_end=tok.pos_end,
                details="Expected int, float, identifier, '+', '-', '(', '[', IF', 'FOR', 'WHILE', 'FUN'",
            )
        )
    ...
    def if_expr_cases(self, case_keyword):
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            # Estamos no contexto de múltiplas linhas, então o resultado não pode ser atribuído numa expressão inline (True passado na tuple)
...
class BaseFunction(Value):
    ...
    def check_and_populate_args(self, arg_names, args, exec_ctx):
        ...
        res.register(self.check_args(arg_names=arg_names, args=args))
        if res.should_return():
            return res

        self.populate_args(arg_names=arg_names, args=args, exec_ctx=exec_ctx)
        return res.success(None)
    ...
    def execute(self, args):
        ...
        res.register(
            self.check_and_populate_args(
                arg_names=self.arg_names, args=args, exec_ctx=exec_ctx
            )
        )
        if res.should_return():
            return res
...
class BuiltInFunction(BaseFunction):
    ...
    def execute(self, args):
        ...
        res.register(
            self.check_and_populate_args(
                arg_names=method.arg_names, args=args, exec_ctx=exec_ctx
            )
        )
...
class SymbolTable:
    ...
    def __init__(self, parent=None) -> None:
        self.symbols = {}
        self.parent = parent  # is another SymbolTable

    def get(self, var_name):
        value = self.symbols.get(var_name, None)
        if value is None and self.parent:
            return self.parent.get(var_name=var_name)
        return value
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        elif op_tok.type == TT_MINUS:
            result, error = left.subbed_by(right)
        elif op_tok.type == TT_MUL:
            result, error = left.multed_by(right)
        elif op_tok.type == TT_DIV:
            result, error = left.dived_by(right)
        elif op_tok.type == TT_POW:
            result, error = left.powed_by(right)
        elif op_tok.type == TT_EE:
            result, error = left.get_comparison_eq(right)
        elif op_tok.type == TT_NE:
            result, error = left.get_comparison_ne(right)
        elif op_tok.type == TT_LT:
            result, error = left.get_comparison_lt(right)
        elif op_tok.type == TT_GT:
            result, error = left.get_comparison_gt(right)
        elif op_tok.type == TT_LTE:
            result, error = left.get_comparison_lte(right)
        elif op_tok.type == TT_GTE:
            result, error = left.get_comparison_gte(right)
        elif op_tok.matches(TT_KEYWORD, "AND"):
